...
namespace UnityEngine.UI.Extensions
{
	...
	public class ScrollPositionController : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
	{
		...
		public int itemIndex { get { return Mathf.RoundToInt (GetLoopPosition (autoScrollPosition, dataCount)); } }
		...
		void Update ()
		{
			...
			else if (!dragging && (offset != 0 || velocity != 0))
			{
				...
				else if (inertia)
				{
					...
					{
						var diff = position - currentScrollPosition;

						int RoundedIndex = Mathf.RoundToInt (currentScrollPosition);

						// bool isMinRound = (currentScrollPosition.Abs () > RoundedIndex.Abs ());

						bool isFromDirection = autoScrollType == AutoScrollType.FromDirection;

						var OffsetIndex = isFromDirection /* && isMinRound */ ? (int) diff.Sign () : 0;

						int ScrollToIndex = RoundedIndex + OffsetIndex;

						if (snapType == SnapType.ByOne)
						{
							ScrollToIndex = prevScrollToIndex + (int) diff.Sign ();
						}

						ScrollTo (ScrollToIndex, snap.Duration);
						prevScrollToIndex = ScrollToIndex;
					}
				}
